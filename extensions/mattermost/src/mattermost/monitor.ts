...
import {
  ...
  isMattermostModelPickerContext,
  ...
  const unregisterInteractions = registerPluginHttpRoute({
    ...
    handler: createMattermostInteractionHandler({
      ...
      authorizeButtonClick: async ({ payload, post, context }) => {
        ...
        // `context` is the HMAC-verified, _token-stripped context; the picker tag inside
        // it is therefore trustworthy (only the gateway can produce a valid `_token` for
        // a context that includes the picker tag).
        const channelInfo = await resolveChannelInfo(payload.channel_id);
        const result = resolveButtonClickChannelAuthorization({
          channelInfo,
          account,
          groupPolicy,
          isPickerContext: isMattermostModelPickerContext(context),
        });
        if (result.ok) {
          return { ok: true };
        }
