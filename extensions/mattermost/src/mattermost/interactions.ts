...
// Secret is derived from accountId + bot token so it's stable across CLI and gateway
// processes and unique per account (two accounts sharing a bot token still produce
// distinct secrets, preventing cross-account token reuse).
...
export function createMattermostInteractionHandler(params: {
  ...
  authorizeButtonClick?: (opts: {
    ...
    post: MattermostPost;
    /**
     * The HMAC-verified context with `_token` stripped. The same value is also
     * exposed on `payload.context` (the inbound payload is sanitized before
     * delivery), so authorizers and handlers do not need to re-strip `_token`.
     */
    context: Record<string, unknown>;
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    // Mint a sanitized payload whose `context` no longer contains `_token`. This is the
    // value passed to all callbacks so they cannot accidentally surface or trust the raw
    // HMAC token from the inbound request.
    const sanitizedPayload: MattermostInteractionPayload = {
      ...payload,
    ...
    try {
      const eventLabel =
        `Mattermost button click: action="${actionId}" ` + `in channel ${payload.channel_id}`;
    ...
    try {
      await updateMattermostPost(client, payload.post_id, {
        ...
        props: {
          attachments: [
            {
              text: `✓ **${clickedButtonName}** selected`,
            },
          ],
        },
      });
